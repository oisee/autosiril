...
* autosiril.rb           - оригинальный рабочий файл (стабильная монолитная версия)
* autosiril_refactored.rb - **НОВАЯ** чистая модульная версия - создаёт 100% идентичный результат!
* main.rb               - старая попытка рефакторинга (сломана, не использовать)
* module_template.rb    - вспомогательный файл, в котором находится шаблон модуля VortexTracker'а

✅ **Рекомендуется**: Используйте autosiril_refactored.rb для разработки - он чистый, хорошо документированный и создаёт **100% идентичный результат** с оригиналом.

✅ **Полная совместимость**: Рефакторинговая версия тщательно протестирована и проверена на создание побайтово идентичных файлов VortexTracker (кроме временных меток и исправления бага с дублированием в PlayOrder).

⚠️ **Легаси**: autosiril.rb остаётся доступным для справки. main.rb сломан и не должен использоваться.

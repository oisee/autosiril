...
class Setup
  ...
  def initialize
    ...
    # Parse --format flag from ARGV before positional args
    @output_format = 'vt'
    format_idx = ARGV.index('--format')
    if format_idx
      @output_format = ARGV[format_idx + 1] || 'vt'
      ARGV.delete_at(format_idx + 1)
...
# ============================================================
# Pipeline functions — extracted from monolithic autosiril.rb
# Each function takes explicit inputs and returns outputs.
# No global state mutation except $set (read-only after init).
# ============================================================

# Step 1: seq2vmod — Convert MIDI sequence to virtual module (array of vchannels)
def seq2vmod(seq)
  vmod = []
  seq.each {|track|
    puts "   track:#{track}"
...
# Step 2: vmod2rmod — Expand virtual notes into per-row FNote arrays
def vmod2rmod(vmod)
  rmod = []
...
# Step 3: detect_key — Detect musical key from note statistics
def detect_key(rmod)
...
# Step 3b: apply_diatonic_transpose — Diatonic transposition (mutates rmod in place)
def apply_diatonic_transpose(rmod, good_key)
...
# Step 4: rmod2pmod — Flatten polyphonic/mono/drum cells
def rmod2pmod(rmod)
...
# Step 5: pmod2lmod — Convert to LNotes with ornaments
# Returns [lmod, ornaments_hash]
def pmod2lmod(pmod)
...
# Step 6: apply_delays — Apply echo/delay effects (mutates lmod channels)
def apply_delays(lmod)
  puts "--- making delay ---"
  dmod = []
...
# Step 8: render_into_text — Render 3-channel lmod into VT2 text lines
def render_into_text(lmod)
...
# Step 9: split_into_patterns — Split text lines into patterns, deduplicate, build play order
def split_into_patterns(text_lines)
  ...
  [wet_patterns, use_patterns, wet_play_order_txt]
end

# Step 10: render_ornaments — Convert ornaments hash to VT2 text
def render_ornaments(ornaments)
  ornaments_txt = ""
...
# Step 11: write_vt_output — Write VT2 module file
def write_vt_output(wet_patterns, use_patterns, wet_play_order_txt, ornaments_txt)
...
if $set.output_format == 'vt' || $set.output_format == 'both'
  ...
  text_lines = render_into_text(lmod_mixed)
  wet_patterns, use_patterns, wet_play_order_txt = split_into_patterns(text_lines)
  ornaments_txt = render_ornaments(ornaments)
  write_vt_output(wet_patterns, use_patterns, wet_play_order_txt, ornaments_txt)
end
